...
[[ $DEFAULT_REPO == "Skeptomenos/omarchy-mac" ]] || {
  echo "not ok - guided install defaults to the supported Apple Silicon fork"
  exit 1
}
[[ $DEFAULT_REF == "quattro-arm" ]] || {
  echo "not ok - guided install defaults to the supported Apple Silicon branch"
  exit 1
}
grep -qF 'git clone --branch "$ref" --single-branch --depth 1' "$TOOL" || {
  echo "not ok - staged clone is pinned to one branch"
  exit 1
}
grep -qF 'clone --branch "$ref" --single-branch' "$TOOL" || {
  echo "not ok - installed checkout is pinned to one branch"
  exit 1
}
