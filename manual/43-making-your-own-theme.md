...
A theme you install from someone else's repo with `omarchy theme install` keeps everything that's colour, and loses the handful of files that would run code or change security-sensitive configuration on your machine: any `.lua` file, the terminal configs (`alacritty.toml`, `foot.ini`, `ghostty.conf`, `kitty.conf`), `hyprlock.conf`, and `vscode.json`. A theme's `hyprland.lua` is Lua your compositor runs at login, a terminal config names the program your terminal starts, `hyprlock.conf` is sourced into the lock screen, and `vscode.json` names a VSCode extension to install. Installing someone's theme should change what your desktop looks like, never what it runs.
...
Remember that once it's installed from a repo, any `.lua`, terminal or lock-screen config, or `vscode.json` it ships is dropped, so don't build the theme around those.
