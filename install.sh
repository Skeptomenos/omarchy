...
install_default_package_set() {
  local requested_package install_package target skipped=() unbuildable=() attempt_unavailable=0
  ...
  while read -r requested_package; do
    ...
    if (( ! attempt_unavailable )) && package_is_unavailable_here "$requested_package"; then
      unbuildable+=("$requested_package")
      continue
    fi
    target=$(omarchy_arm_default_package_target "$install_package")
    yay -S --needed --noconfirm "$target" </dev/null || skipped+=("$requested_package")
