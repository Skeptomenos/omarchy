...
```bash
curl -fsSL https://raw.githubusercontent.com/Skeptomenos/omarchy-mac/quattro-arm/bin/omarchy-mac-setup | bash
```
...
```bash
curl -LO https://raw.githubusercontent.com/Skeptomenos/omarchy-mac/quattro-arm/bin/omarchy-mac-setup
bash omarchy-mac-setup --no-encrypt
```

(`--repo <owner/repo>` installs from a fork the same way. `quattro-arm` is the
supported Apple Silicon release branch and the script installs from it by default; it
...
**Mind the branch.** The command above pins the supported Apple Silicon
release branch. A plain clone uses the repository default branch, which is not
the release used by this fork. `cat version` is how you check before committing
to it.
...
- DEV-147 front-port display result — [docs/evidence/dev-147-fairydust-front-display-2026-09-05.md](docs/evidence/dev-147-fairydust-front-display-2026-09-05.md)
