...
- `hyprlock.conf` — the Mac fork sources it into the lock-screen configuration
...
- Hand-written overrides work everywhere except a `.lua`, terminal or lock-screen config, or a `vscode.json` in a theme cloned from a git repo; see [What an installed theme may not ship](#what-an-installed-theme-may-not-ship).
