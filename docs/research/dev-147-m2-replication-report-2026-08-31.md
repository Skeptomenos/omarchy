...
| [Later USB-only loss on that W boot](../evidence/dev-147-lg27-usb-data-loss-2026-08-31.md) | Hub/controls disconnect twice; first recovery succeeds, second fails with descriptor/address `-71`; the later snapshot retains both native outputs and both PD sources | Physical-change history and cause are not yet established; mouse test held |
