...
## Addendum — action report and earlier suspend failure, 2026-08-31 11:06Z

David answered “yes” to the compound question about reconnecting during 12:15–12:18 CEST and image return. The agent initially interpreted both parts as confirmed. The exact answer is preserved, but its association with a new versus earlier reconnect remains uncorroborated. The agent asked which event David meant and released no repeat.

A separate earlier-gap read found hub/controls enumeration at 11:57:09 CEST, loss at 11:58:16, and repeated enumeration failures through 11:59:11.171. It contains ten identified hub generations and six controls generations, each with a disconnect. There is no DP, Type-C or USB-bus recreation event in that segment. Do not assign these events to a physical action or call them spontaneous without testimony.

At 11:58:56.443–.444, hub generation 22 reports a transaction-translator error `-71` and a hub-status/configuration failure `-5`. At .454196, xHCI rejects a suspend request because the port does not meet its enabled/link-state guard. At .455007 the hub reports failed suspend `-32`; it disconnects at .457182. Thus this generation already had errors before the suspend warning. The last USB event precedes the PM recorder by 952.762 seconds; no child PM counters cover it.

The [pinned xHCI branch](https://github.com/AsahiLinux/linux/blob/e2e1930a9595bffafad92cec2b5504525efb9cd4/drivers/usb/host/xhci-hub.c#L1206-L1235) checks port-enable, reset and link-state bits before its U3 suspend write. The [USB-core error path](https://github.com/AsahiLinux/linux/blob/e2e1930a9595bffafad92cec2b5504525efb9cd4/drivers/usb/core/driver.c#L1361-L1395) clears non-AUTO suspend errors for non-root devices before this diagnostic, which also requires a failed status read. These messages therefore support a failed runtime/autosuspend attempt on an already failing hub. They do not prove successful suspension or the initiating cause; no PM workaround was applied.

The [interface-probe error cleanup](https://github.com/AsahiLinux/linux/blob/e2e1930a9595bffafad92cec2b5504525efb9cd4/drivers/usb/core/driver.c#L374-L401) can itself request autosuspend. This is a source-compatible explanation for a suspend attempt after the hub configuration failure, not a captured call-stack result. Independent source review agrees with this distinction.

Independent saved-data QA PASS: `sha256sum -c capture.sha256` verifies 21/21 new raw files. The earlier journal has 407 rows/342,727 bytes; its last entry exactly matches the PM capture's anchor. The later journal has 410 rows/367,075 bytes through 13:03:07.532 CEST, all audit/network entries, with no USB/display event. Both cursor continuations, boot identity, ordering, exit 0 and empty stderr checks pass. Earlier/later SHA-256: `a948de818fb0e95aaed8034d0a1e088126708abb8036b759f083b18d7ee4a1cd` / `d1bfdd562df62cc0d05bdf22e90c6baf4a2cb14300e818761b2c6c5c61e07914`. The 13:03:57 same-boot snapshot retains both native outputs/DPMS, both PD sources and Full/100% battery. Physical timing and the child-PM measurement remain open; the original records are unchanged.

## Correction — no new reconnect, 2026-08-31 11:13Z

David explicitly clarified that he had not removed the monitor or changed anything in the last hour. He then explained that “yes” referred to the earlier completed test: unplug, wait five seconds, reconnect, then external video returns after about ten seconds. It does not confirm a new action during the 12:15–12:18 CEST PM recording. The agent's initial interpretation caused the confusion and is withdrawn.

The recorder therefore captured a no-action baseline, not a reconnect test. Its 180 root-only samples and the earlier successful video recovery remain valid, separate records. No child-PM transition was measured. The last-hour statement does not establish what happened during the older 11:57–11:59 USB failures. The timing question is resolved; do not ask it again or release a repeat. This correction changes documentation only. The open fault concerns USB-hub/data reliability while video works; it is not evidence of a new display failure.
