...
David confirms one insertion followed by no cable handling throughout the 45-second capture. Thus the repeated controller-reported connection transitions did not represent repeated user insertions. This does not distinguish PD negotiation retries from electrical/link instability. Next bounded control: reverse only the Mac-end plug orientation, keep LG27 powered on and its input/monitor-end/rear connections fixed, and make one attachment during an `attach` capture. Record orientation as a relative reversal, not an absolute pin orientation. Any recovery remains confounded by the intervening disconnect/dwell; one result alone cannot prove an orientation defect.

## Addendum — reversed LG27 orientation also lacks DP, 2026-09-05

Capture `trace-capture.LGwIlYwJ` on the same boot runs from 3727.72 to 3772.72 seconds. Exit 0, empty stderr, instance removed, 31/31 records and all 24 loss counters zero. Report SHA-256: `899094cf075131e0d17720e17c67e426b6712b959c70a51ae7b5277b95bb7477`.

Front data events at 3742.910884–3743.948295 include DATA_UPSIDE_DOWN, confirming the relative orientation changed from ZU4DfANM. They retain USB_DATA_ROLE and never report DP/HPD. No filtered IOMFB pushes occur. Snapshot `lg27-reversed-orientation.ut2ki8dw` confirms external disconnected/disabled and internal connected/enabled, with zero collection issues and zero classified records in its recent journal window. This quiet snapshot does not erase earlier boot faults. Front remains device/PD sink; the corresponding kernel interval records DPTX disconnects without a later modeset.

Both tested orientations lack DP in the current state. This weakens a simple single-orientation explanation but does not isolate cable, monitor or host state. Asked David to confirm the visible result and whether a second cable already known to carry USB-C video is available. Leave the current connection fixed pending that answer; no additional identical reconnect or speculative kernel patch.

## Addendum — role-path source analysis, 2026-09-05

David requests continued investigation using the same cable, which already carried 4K video and USB in this boot. No cable defect is established. Direct source comparison finds trial `drivers/usb/typec/tipd/core.c` and retained W `/home/david/o/.dev147-stage/work/tps6598x-core/core.c` byte-identical, SHA-256 `bb19187a1c41517e4b9f0fc3da7089fd41d26851774001ae3ad10c43139f2e15`. Substituting that W file would make no change.

In the frozen trial source, `tps6598x_read_data_status` reads the controller register and emits the data trace before the deferred worker runs. The CD321x worker uses a 500 ms debounce and translates the reported role into USB-role requests. It transitions an active role through NONE when needed, handles disconnection/PHY safe state, updates connector properties, programs the mux, then requests the new role and emits HPD. No explicit data-role-swap command is issued by that normal worker; SWDF belongs to the separate user role-change path and must not be retried here.

The observed USB_DATA_ROLE bit is therefore a controller report, not a value invented by the display timeout patch. It is not proof of the final DWC3 role: when USB2/USB3 connection flags are absent, the worker requests NONE. Shared teardown and prior host/controller state can still influence later behavior. The existing trace does not expose worker return values or full power/connection status, so it cannot identify the underlying cause or justify a speculative ordering fix.

CD321x already binds `tps6598x_status` and `tps6598x_power_status` tracepoints in this kernel. Prepare a separate negotiation collector that requires those events before READY and retains the existing data/IRQ/DCP capture and loss checks. One monitor power-button off/on sequence keeps all USB cables and the host boot fixed. This tests monitor-only recovery and records richer status without a kernel rebuild. A power button may leave the PD controller powered; an unchanged trace is not proof of host failure. Generic status fault labels alone must not be treated as verified electrical faults.

Independent source comparison also confirms identical W/trial TIPD header and trace files, Apple DWC3 and ATC PHY code. Receipt `acceptance/diagnostic-design/source-comparison.fvvcjas6/review.json`, SHA-256 `57912295b4b2a26fa4abcdcd47b6662efa1b4db30e70887df58ef5729c55b20d`, records fresh checks of Asahi branches. No applicable ordinary role/debounce recovery fix was found in the checked newer code; its relevant changes concern Thunderbolt switch/VDO integration. This is not an audit of every submitted patch. Ignored mux/role return values and mode caching before callback remain inherited diagnostic gaps, not established causes of the controller's no-DP report.

## Addendum — monitor-only negotiation collector prepared, 2026-09-05

The separate `clear-wait-trial/negotiation/trace-capture.sh monitor-power` accepts only that explicit mode and the current trial release. It requires the full status and power events before READY, preserves external DCP filters and optional data/IRQ events, and retains per-CPU loss and cleanup reporting. The user action is one LG27 power-button off, five-second wait and power-on with all USB cables fixed. No script-triggered reset or live trace ran during preparation.

Capture SHA-256: `b25a2b42c8dd1fc4659a99ba9e76b89dcb8cc04fa10387c44b5cf3cc1504ef8d`. Author gate `negotiation/checks/software.tviXJcdh` exits 0 with eight methods in 0.745 seconds. Independent gate `software.O9kLDx2T` also exits 0. Both run the actual launcher and payload with explicit uname/sudo/tracefs/terminal/timer/removal fixtures, plus lint, strict types and shell syntax. Required-event absence, invalid mode/release, filter errors, loss reporting and cleanup/signals are covered. This does not prove real event delivery or root-cause recovery.
...
David confirms no image after the B9GNLQls monitor-power control. This closes the pending visual observation: controller-visible disconnect/reconnection did not restore video. Both plug orientations and this monitor-power control have failed in the current boot state. These are not independent cold-start trials and do not isolate host versus monitor/controller state. Preserve the connected setup; next diagnosis must address negotiation/role handling before further display-timeout tuning.

## Addendum — X8 enumerates during investigation, 2026-09-06
