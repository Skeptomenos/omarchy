...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Current dependency: first trial reconnect restores image after about 11 seconds but does not invoke clear-swap. Monitor USB hub fails enumeration. Capture one front-only reconnect from the now-active display, rear cable untouched, to test the clear-wait and shutdown tail. Review before another cycle; no endurance batch. The saved default remains fairydust1.
