...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Current dependency: LG35 comparison uhjc27vs produces no image and reports front USB device role without DP/HPD. Shutdown completes in 48.220 ms; this is a negotiation failure, not clear-wait timeout. DATA_UPSIDE_DOWN differs from the working trace. Await LG35 input/cable/port/orientation details before the next discriminator. Retain the trial and original default; no stress batch or repeated SWDF write.
