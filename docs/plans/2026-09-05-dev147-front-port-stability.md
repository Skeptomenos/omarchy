...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Manual dependency: runtime inventory completed successfully. Prepare and independently validate `dev/apple-dp-altmode/fairydust/acceptance/trace-capture.sh`, then David runs it with his password and performs one front-port reconnect after READY. Use one private trace instance, 45 seconds, verified external-controller filters and per-CPU loss statistics. Missing events, setup errors, dropped records or incomplete cleanup must remain explicit. Do not infer a clean fault result from launcher exit 0.
