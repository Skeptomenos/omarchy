...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Current dependency: user authorizes comparison with the earlier working W stack; another LG35 reconnect remains blank. Capture current failure, verify exact W image, and use the existing paired restore launcher. David performs selected-file changes and reboot. Old default initramfs is not W: select its exact dpalt image only after old bundle restoration and full restart. No stress batch or repeated SWDF write.
