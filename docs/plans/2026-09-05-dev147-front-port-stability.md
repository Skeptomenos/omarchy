...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Manual dependency: read-only trace capability inventory. `/sys/kernel/tracing` is root-only; the agent must not run sudo. Prepare and review `dev/apple-dp-altmode/fairydust/acceptance/trace-preflight.sh`, then David runs it to capture actual event formats. Do not enable tracing or request cable action at this preflight stage.
