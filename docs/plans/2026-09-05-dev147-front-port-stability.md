...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Current dependency: W/trial role-path source is identical and no applicable fix was found in checked newer branches. Separate negotiation collector passes author/independent gates. David runs `negotiation/trace-capture.sh monitor-power` and, only at READY, powers LG27 off, waits five seconds and powers it on once with all USB cables fixed. Inspect full status/power events, loss counters and visual outcome before another action. No cable substitution, role forcing or kernel rebuild.
...
- [ ] Regression comparison: retained W versus current trial.
  Goal: establish whether the same LG35 setup works on the earlier stack before adding another patch.
  Assumption: retained W image and its paired old bundle still match the recorded working identities.
  Verify: user-run SHA-256 check of root-private W against ae8f1ed7f4f258f89931209cd7de6030be9f6875372d7329151b822a6ba2281f; existing restore verifies old bundle 203ab702 and original GRUB. Independent review must verify restore remains valid from the trial.
  ...
  Negotiation probe: the device-role bit is traced directly after the controller register read, before the deferred role switch. Add required existing tps6598x_status and tps6598x_power_status events in a separate capture tool. Assumption: they expose connection/role and power changes on the current kernel; the source binds both tracepoints to CD321x, and the launcher must refuse before READY if either is unavailable. One later attended monitor off/on sequence keeps USB cables and the host boot fixed. Software gates must cover required-event failure, prompts, cleanup and loss counters. Recovery alone cannot prove a monitor fault or repair the driver.
...
- 2026-09-05: Actual restore-results.77wrOH2z returns zero, empty stderr, matching old bundle and original GRUB; fresh W checksum passes. [Restore addendum](../evidence/dev-147-clear-wait-trial-preparation-2026-09-05.md) records the receipt. Next full restart and exact W initrd selection; trial artifacts remain preserved and loaded trial stays active until restart.

- 2026-09-05: User requests older-stack comparison and reports another LG35 reconnect still blank. Snapshot lg35-repeat-failed.mpt6_vtg confirms current trial DP disconnected. Retained W hash matches recent root receipt and historical startup evidence; original GRUB default points to a different initramfs. Prepare a fresh W checksum followed by reviewed paired restore, with no reboot until receipt review. A later matched fresh trial start is required for attribution.

- 2026-09-05: LG35 has no image. Loss-free uhjc27vs trace records front device role/no DP/HPD with different reported orientation; front host buses are absent. The previous display shuts down without timeout. [LG35 addendum](../evidence/dev-147-clear-wait-trial-preparation-2026-09-05.md) records the confound and pending input/cable details. Do not interpret this as a clean monitor-only USB comparison.

- 2026-09-05: User confirms H6RTWN8n visible recovery in roughly 5–7 seconds. The bounded clear-wait probe is complete with mechanism and visual evidence. USB isolation is next; query available comparison cable/monitor before further physical actions. Type-C status remains unchanged after 1262.711295 while later USB failures persist; no causal cable/hub/driver conclusion. Earlier USB-before-teardown timing does not locate the physical unplug.

- 2026-09-05: H6RTWN8n captures actual poweroff clear-swap replies at 53.221 ms, followed by abort/last-close/power-state ACKs and poweroff done with no timeout. This supports the larger budget in one observed case. Both DRM connectors return; user-visible confirmation remains pending. [Longer-wait addendum](../evidence/dev-147-clear-wait-trial-preparation-2026-09-05.md) records the loss-free trace and repeated USB faults. No stability or production-bound claim; stress cycles remain paused.
