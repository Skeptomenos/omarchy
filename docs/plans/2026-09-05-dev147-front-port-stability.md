...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Current dependency: H6RTWN8n exercises a 53.221 ms clear-swap reply chain and completes the shutdown tail without timeout; user confirms image recovery in 5–7 seconds. Retain the 100 ms trial. USB hub repeatedly fails despite settled Type-C/DP state and is absent. User has LG35: capture one monitor substitution using the same cable/front port/orientation, rear untouched, current boot. Compare visible image and USB enumeration; no endurance batch. The saved default remains fairydust1.
