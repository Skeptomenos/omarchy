...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Manual dependency: rear-empty reconnect succeeded, and rear X8 insertion left front DP enabled but produced a monitor-hub interruption. The X8 is USB 3.2 Gen 2; it has not enumerated as a USB device. David now runs `trace-capture.sh` without an argument and performs one front reconnect at READY, leaving the X8 and its rear cable untouched. Keep the same boot and orientation, report visual outcome, and stop on failure. This compares rear occupancy, not storage traffic.
