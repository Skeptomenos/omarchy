...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Current dependency: fresh W checksum passes and paired restore returns RESTORED_NOT_REBOOTED. Selected ESP and original GRUB now match old pins; current running kernel is still the trial. David fully restarts, selects Arch Linux and adds -dpalt to the initrd basename only. Preserve LG35/cable/rear setup; report release and physical display outcome before reconnect. No stress batch or repeated SWDF write.
