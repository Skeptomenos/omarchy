...
- [ ] Slice 2: Prove front-port reconnect endurance.
  ...
  Manual dependency: targeted reconnect trace completed with failed image recovery and no record loss. Independently validate attach mode in `dev/apple-dp-altmode/fairydust/acceptance/trace-capture.sh`, then David power-cycles the monitor while USB-C is disconnected, runs the launcher with `attach`, and connects once after READY. Use one private trace instance, 45 seconds, verified external-controller filters and per-CPU loss statistics. Missing events, setup errors, dropped records or incomplete cleanup must remain explicit. Do not infer a clean fault result from launcher exit 0.
