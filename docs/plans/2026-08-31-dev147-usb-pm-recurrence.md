...
Status: offline preparation complete; reader unarmed. User availability and the new attended capture remain pending. No child PM result or repair is claimed.
...
- Once preparation is reviewed, arrange one attended capture: MagSafe, lid open, same monitor/cable/front port and empty monitor USB-A ports. Before arming, check current boot, kernel/module identity and that the fixed DWC3 root exists; stop on drift. Start the recorder first, then request one reconnect. Keep input/settings unchanged. Stop on internal/system/power regression or after the one case. If the user is not ready during the bounded window, retain that no-action result rather than repeat automatically.
