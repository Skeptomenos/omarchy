...
- [ ] Slice 7: keep DisplayPort usable across repeated AFK service generations.
  ...
  - [ ] Run one contained host-role diagnostic on the current candidate boot while the cable remains unchanged. Use an isolated trace instance with a bounded buffer and timeout. Arm the existing TIPD tracepoints. Immediately before the write, verify that `/sys/class/typec/port1` still resolves to controller `0-003f`. Write `host` once to its `data_role` and record the exit status. Validate the returned role state, xHCI and UDC presence, DP and HPD state, physical image, internal display, system responsiveness, and bounded trace. Clean up the trace instance on every exit. Do not change a boot file or module. Keep recovery through `/boot/initramfs-linux-asahi-m2-displayport.img` ready.
    ...
    Verify: compare `/sys/class/typec/port1/data_role`, xHCI device presence, `/sys/class/udc`, `DP-1` status and enabled state, HPD events, and the physical image before and after the single request.
  Validation:
