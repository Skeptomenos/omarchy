...
- [ ] Slice 7: keep DisplayPort usable across repeated AFK service generations.
  ...
  - [ ] When separately authorized, run exactly one attended non-default boot with MagSafe connected and every external USB-C device disconnected. Select `/boot/initramfs-linux-asahi-m2-displayport-afk-reuse.img`, then verify the internal display and system responsiveness before attaching a monitor. If the candidate fails, recover by selecting `/boot/initramfs-linux-asahi-m2-displayport.img`. Keep the ten-generation test as a separate later gate.
  Validation:
