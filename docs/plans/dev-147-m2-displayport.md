...
Updated: 2026-08-28, after [functional display recovery following the intended W restart](../evidence/dev-147-w-recovery-after-e-2026-08-28.md). Both native outputs are active on a fresh boot, and David reports the external image restored. He did not repeat the filename in this report; shared W/E module IDs do not independently prove W artifact startup. The W handoff is consumed. The [confirmed E failure](../evidence/dev-147-c4-selection-confirmation-2026-08-28.md) remains separate, with cause unknown. Root hubs only enumerate; full Gate 4b/USB acceptance stays HOLD. The reported login-focus issue is a separate possible greeter hotplug/focus bug, not a confirmed primary-monitor switch or reproduced authentication bug. No further device or greeter test is authorized. [C3 staging](../evidence/dev-147-usbearly-staging-2026-08-28.md) remains consumed with qualified root-private provenance. D3 remains external-display FAIL / measurement INCONCLUSIVE. [Linear DEV-147](https://linear.app/helmus/issue/DEV-147/usb-c-displayport-external-monitor-unavailable-on-m2-macbook-air) owns issue status and dated checkpoints.
...
The W handoff is consumed. Root hubs alone enumerate; USB/full Gate 4b remains HOLD. Known firmware diagnostics remain, and neither reliability nor E/D3 causality is established. Preserve all images, backups, and evidence. No fresh reboot, reconnect, sudo command, USB-device test, mode change, suspend, module/helper operation, or greeter test is authorized. Normal boot still does not restore the original DTB; Mac restore execution remains untested.
