...
Reconciled: 2026-08-29, after the [battery-depletion incident](../evidence/dev-147-battery-depletion-2026-08-29.md) and [physical-power addendum](../evidence/dev-147-battery-depletion-addendum-2026-08-29.md). MagSafe was disconnected overnight; the monitor was the only intended source and was later found in standby. All candidate boots and T1 continuation are on safety HOLD. The machine is on the default stock-core boot and charging with MagSafe plus the monitor connected. Omarchy Stay Awake remains enabled; keep MagSafe connected and do not leave the running machine unattended on battery until `omarchy toggle idle allow-idle` and its status check pass. Do not select a DP/USB diagnostic image or resume T1 while this hold is active. The earlier [A0 findings](../research/dev-147-hpd-startup-2026-08-29.md), [T1 selection](dev-147-usb-startup-diagnostic.md#a1-selection--t1-tipd-sender-diagnostic-living), [functional display recovery](../evidence/dev-147-w-recovery-after-e-2026-08-28.md), [confirmed E failure](../evidence/dev-147-c4-selection-confirmation-2026-08-28.md), and D3 failure remain valid historical records with their stated limits. No T1 image exists or is ready to select. Firmware questions, automatic monitor-USB enumeration, power safety, reliability, and full Gate 4b stay HOLD. The login-focus issue remains outside this goal. [Linear DEV-147](https://linear.app/helmus/issue/DEV-147/usb-c-displayport-external-monitor-unavailable-on-m2-macbook-air) owns issue status and dated checkpoints.

Public-copy boundary: source and reviewed notes use `codex/dev-147-m2-dp-altmode-public`. The initial checkpoint `c781312d1221da675e686b318a30ddd10d9ef3c4` passed remote readback; `7c010fcac87777d8cd674ec9fd823ee85ae89766` was the last verified head before this continuation. Accepted source commit `0303e2adf` identifies the pure E recipe and bounded capture package. This living document does not embed a mutable final branch head; Linear owns the exact current publication/readback checkpoint. The original branch and its 14 local commits remain private and unchanged. Boot IDs are redacted here; raw Linear exports and machine records are excluded. The older public boot/staging helpers have invalid machine identifiers and are not the byte-identical, tested operational copies. The later C2 exports preserve exact workload bytes but require private sandbox inputs. Do not run any of these helpers live. See the [source archive](../../dev/apple-dp-altmode/usbdiag/README.md) and [dated checkpoint](../evidence/dev-147-public-source-checkpoint-2026-08-28.md).
...
- David runs every privileged command. Before a future mutation, review its exact command, rollback, file targets, and evidence checks with him. He must be present, with saved work and battery at least 80%, preferably full.
- Incident override: no candidate boot is currently authorized. Keep the default stock-core boot and MagSafe connected. A later proposal must implement and pass the [reviewed power-guard design](../research/dev-147-power-guard-design-2026-08-29.md). It requires both the fixed MagSafe source and aggregate AC online, coherent fresh charging data, a 15-minute same-topology stock baseline, at least 80% battery, and a one-shot candidate window of at most 10 minutes with David present and a separate-device timer. Abort on any source loss, input-limit regression, discharging or contradictory state, a fall greater than one point, or a late, missing, or malformed sample. Use a visible normal reboot to the unchanged stock default while Linux and the internal screen are responsive. No unresponsive-candidate recovery bridge is available yet. This missing bridge blocks release. Never type blind commands or depend on candidate Type-C or `s2idle`. Do not rely on monitor USB-C power.
- Before releasing the hold, independently review focused tests and source/binary evidence that the candidate preserves stock Type-C and USB-PD paths. The static audit found no direct PD or charging-path edit, but it found an indirect serialized-callback delay risk and added display-complex power use. The final handoff must keep stock as the automatic default and must test its finite timer, telemetry refusal, responsive normal-reboot route, and separate visible unresponsive fallback without selecting the candidate. The existing Recovery guide lacks the physical hung-Linux-to-visible-stock-selection bridge, so candidate release remains blocked until that bridge is written and reviewed.
...
| Battery-depletion incident | Safety HOLD | MagSafe was absent. Monitor-only input stopped supplying enough net power, the awake candidate session drained to 2%, and low-battery `s2idle` followed; the retained journal has no later resume or orderly shutdown, and the next boot was unclean. The exact monitor/PD interruption cause is unresolved. Current default boot charges with MagSafe. No candidate boot or unattended candidate session is authorized. |
| Autonomous offline goal | Paused after partial A2 foundation | [T1 foundation evidence](../evidence/dev-147-t1-offline-foundation-2026-08-29.md) records the accepted work through the bounded capture package. Semantic RED continuation was stopped before execution. The real E control, T1 image, collector binding, A3/A4 package, and manual handoff do not exist. Resume only after the incident hold is reviewed and explicitly released. |
| Source publication trail | Live branch and Linear own current state | Accepted source commit `0303e2adf` and this reconciliation follow the previously verified `7c010fcac` checkpoint. Linear owns the exact current remote head/readback. Raw private records and the original local history remain private. This is not a build or hardware acceptance pass. |
...
<a id="current-state--display-recovery-observed-living"></a>

### Current state — default boot and power-safety HOLD (LIVING)
...
The W handoff is consumed. The later unattended candidate session ended in the [battery-depletion incident](../evidence/dev-147-battery-depletion-2026-08-29.md). The current machine boot is the default stock-core image. Its internal display is usable and it is charging with MagSafe plus the monitor source online. External video is unavailable on this stock core, which is expected and is not a reason to select another image now.

The saved USB snapshot has root hubs only; USB/full Gate 4b remains HOLD. Known firmware diagnostics remain, and neither reliability nor E/D3 causality is established. Preserve all images, backups, and evidence. All T1 work and candidate boots are paused. Do not reboot for external video, reconnect for a test, run sudo, add a USB device, change modes, suspend, use a live module/helper, replay an old handoff, or test the greeter. Normal boot still does not restore the original DTB; Mac restore execution remains untested.

Historical user report, 2026-08-28: the monitor charged the MacBook Air over USB-C without MagSafe during an observed session. The later incident supersedes any operational reliance on that report. David confirmed that MagSafe was disconnected overnight, the powered monitor was the only intended source, and the monitor was later found in standby. This does not identify why monitor power became inadequate. Keep MagSafe connected now. Before any future candidate case, establish a same-topology stock baseline and an attended real-time power guard.
...
Current offline step, 2026-08-29: PAUSED by the battery-depletion safety hold. T1's [offline foundation](../evidence/dev-147-t1-offline-foundation-2026-08-29.md) remains the last accepted checkpoint. A later semantic RED draft was rejected before execution. The actual 424-child E control, T1 image, fixed collector binding, A3/A4 package, and manual handoff do not exist. Do not resume them until the incident documentation and power guard pass review and David explicitly releases the T1 hold. The [subplan](dev-147-usb-startup-diagnostic.md#a2-foundation--continued-offline-preparation-living) owns the retained sequence.

Historical goal, currently paused: reach the furthest evidence-led offline outcome for dependable video and monitor-USB startup on the existing front/lower M2 path, while preserving the working setup. Deliver one fully reviewed next-test package, or an independently reviewed NO-GO dossier if an indispensable observation or authority is missing. David had authorized progress without manual intervention before the battery incident. That authority is suspended. The [diagnostic subplan](dev-147-usb-startup-diagnostic.md#autonomous-offline-technical-gates-living) retains the technical gates for possible later use.

Retained future rule: all new artifacts and raw evidence use fresh private directories with fixed tool/input pins. Do not edit old seals, operational helpers, images, or backups. Read-only source and named benign attributes are allowed; never read partner `usb_mode`. No new build, test, helper, T1, or candidate execution is currently authorized. If the hold is later released, bounded offline execution is allowed only in a fresh verified unprivileged sandbox with read-only inputs and private outputs. Kernel/dependency drift stops work; no installation, silent repinning, or unrestricted fallback is authorized.
...
- 2026-08-29: Put all candidate boots and T1 continuation on safety HOLD after the unattended candidate session lost net external charging, drained from full to 2%, and entered low-battery `s2idle`; the retained journal has no later resume or orderly shutdown, and the next boot was unclean. David later confirmed that MagSafe was absent and the powered monitor was the only intended source; it was found in standby. This narrows the event to unreliable monitor-only power without proving a monitor, cable, firmware, or patched-driver cause. Keep the current default stock-core boot and MagSafe charging. Preserve all artifacts. A later live proposal must pass the exact 15-minute stock and 10-minute candidate power guards in this plan. The first resumed live case is a power-safety diagnostic, not display or USB acceptance.

- 2026-08-29: Accept the pure E-recipe and bounded capture-package checkpoints after independent review. Preserve the recipe's 16-method zero-child model result, the capture review's two-assertion RED, and the corrected 21-method zero-child consistency result. These do not execute the 424-command E control, collect a journal, bind an accepted image, or prove hardware behavior. Continue real E control, one T1 image, operational capture activation, then A3/A4. No manual action follows.
...
- 2026-08-28 (superseded by the 2026-08-29 power-safety hold): Define the user-authorized autonomous offline goal with two reviewed outcomes: one tested package ready for the first manual boundary, or a substantiated NO-GO dossier naming an indispensable missing observation/authority and minimal next action. Select one evidence-justified candidate or diagnostic only after review; retain fixed pins, fresh containment, RED/GREEN, QA, safety review, seals, own-branch publication, and DEV-147 sync. No live change, E retry/B/G ladder, greeter fix, or hardware-PASS claim follows. The user-attested USB-C-only charging result was not a controlled power test or current-configuration verification. Its former no-MagSafe guidance is withdrawn.
