...
- Incident override: no candidate boot is currently authorized. Keep the default stock-core boot and MagSafe connected. A later proposal must first show a 15-minute same-topology stock baseline immediately before the one-shot reboot, sampled every 60 seconds, with MagSafe online, state `Charging` or `Fully charged`, and a non-decreasing battery percentage. The candidate case must start at 80% or more, preferably full; last at most 10 minutes; use a separate-device timer; sample immediately after userspace and every 30 seconds; and keep David present with suspend disabled for the case. Abort on `Discharging`, MagSafe loss, a fall greater than 1%, or a missing sample. Use a normal reboot to the unchanged stock default while Linux and the internal screen are responsive. Otherwise stop and follow the reviewed visible recovery fallback; never type blind commands or depend on candidate Type-C or `s2idle`. Do not rely on monitor USB-C power.
- Before releasing the hold, independently review focused tests and source/binary evidence that the candidate preserves stock Type-C and USB-PD paths. The final handoff must keep stock as the automatic default and must test its finite timer, telemetry refusal, responsive normal-reboot route, and separate visible unresponsive fallback without selecting the candidate.
...
| Battery-depletion incident | Safety HOLD | The candidate session changed from fully charged to discharging, drained to 2%, and entered low-battery `s2idle`; the retained journal has no later resume or orderly shutdown, and the next boot was unclean. Cause is unresolved. Current default boot charges with MagSafe. No candidate boot or unattended candidate session is authorized. |
...
Historical user report, 2026-08-28: the monitor charged the MacBook Air over USB-C without MagSafe during an observed session. The later incident supersedes any operational reliance on that report. It was not a controlled power test and does not prove unattended or candidate-session safety. Keep MagSafe connected now. Before any future candidate case, establish a same-topology stock baseline and an attended real-time power guard. David's still-open physical clarification is whether MagSafe had been disconnected before the machine was left and whether the monitor stayed powered or entered standby.
...
- 2026-08-29: Put all candidate boots and T1 continuation on safety HOLD after the unattended candidate session lost net external charging, drained from full to 2%, and entered low-battery `s2idle`; the retained journal has no later resume or orderly shutdown, and the next boot was unclean. Keep the current default stock-core boot and MagSafe charging. Preserve all artifacts. Do not infer a TPS6598x cause until the prior MagSafe/monitor state is clarified. A later live proposal must pass the exact 15-minute stock and 10-minute candidate power guards in this plan. The first resumed live case is a power-safety diagnostic, not display or USB acceptance.
