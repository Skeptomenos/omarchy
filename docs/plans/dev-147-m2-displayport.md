...
Updated: 2026-08-28, after [C4 read-only readiness checks](../evidence/dev-147-usbearly-boot-readiness-2026-08-28.md). Physical setup, saved work, and the Mac guide on another device are confirmed; the machine checks pass. The current section below prepares one conditional E boot and a separate conditional W recovery; neither has occurred. Final task release, availability/read-through of the new E/W instructions outside Linux, and choosing this exact case remain prerequisites. E is staged but UNBOOTED. [C3 staging](../evidence/dev-147-usbearly-staging-2026-08-28.md) remains complete and consumed, with qualified user-validator provenance; do not replay it. The last validated [recovery](../evidence/dev-147-dp-recovery-2026-08-28.md) remains one display PASS with packaged DWC3/ATC and the working patched DP core. Full Gate 4b/USB acceptance stays HOLD. D3 remains external-display FAIL / measurement INCONCLUSIVE; its frozen v1 guard is unfixed and video cause unknown. Both old boot handoffs remain consumed. This document owns the exact conditional procedure, not blanket reboot permission. [Linear DEV-147](https://linear.app/helmus/issue/DEV-147/usb-c-displayport-external-monitor-unavailable-on-m2-macbook-air) owns issue status and dated checkpoints.
...
| C3 — E-only staging | Complete; user-run staging PASS | All 41 visible initial-preflight rows match. David's complete final report evidences the later private checks; independent readable hashes and metadata agree. E is staged/unbooted. Handoff consumed; no retry. |
| C4 — one-time E proposal | Prepared conditional handoff; not executed | Physical setup, saved work, Mac-guide access, machine checks, readiness QA, and exact handoff safety review are confirmed. Final task release, the new E/W instructions available/read outside Linux, and choosing the case must precede action. W recovery is conditional. |
...
### Current C4 handoff — one user-selected E boot (LIVING)
