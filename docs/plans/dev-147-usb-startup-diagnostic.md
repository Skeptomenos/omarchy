...
Updated: 2026-08-28, during public-source checkpoint preparation. Status: D1 in progress; the initial isolation check passed. David approved offline implementation, focused QA, and private module/image preparation. Source drafts and schema are saved; the frozen trace/image stubs now have their first isolated RED results. No implementation, diagnostic module, or image is validated. D0 design review remains complete. Finish the reviewed public checkpoint, then resume offline implementation, tests, and private builds. Staging and boot remain unauthorized.

Current source branch: `codex/dev-147-m2-dp-altmode-public`. The [public archive](../../dev/apple-dp-altmode/usbdiag/README.md) preserves the authored drafts and fixtures, but excludes host manifests and raw logs. Public helpers have invalid machine-identity/path placeholders and must not run live. The original operational helpers and private branch remain unchanged. The [dated source checkpoint](../evidence/dev-147-public-source-checkpoint-2026-08-28.md) distinguishes historical R4 evidence from the new private RED runs.
...
| D1 — offline implementation and build | Approved scope; isolation PASS; initial RED observed | R4 and a fresh equivalent probe passed. Frozen trace/image stubs fail their first isolated fixture runs as expected. Implementation, GREEN tests, control/diagnostic builds, and image checks remain pending; no hardware-test pass. |
...
The saved working archive contains 199 modules, including `phy-apple-atc`, `dwc3`, `udc-core`, and the xHCI modules. It does not contain `dwc3-apple`. Its direct dependency closure is already present. The proposed image therefore replaces ATC, adds the diagnostic DWC3 glue, and updates necessary module indexes. Expect 200 modules, not a two-file replacement with an unchanged file set. The DP core stays byte-identical to the successful candidate.
...
Proposed preparation route: use a new private, independent copy of the verified working archive's contents, replace/add only the named modules, and repack into a new image. Generate indexes in a separate private, reduced module staging root. The saved image has 199 dependency records and seven indexes; the full private tree has 1,861 dependency records. The image lacks `modules.order`, `modules.builtin`, and `modules.builtin.modinfo`. Supply those exact saved, pinned inputs to the staging root; do not run bare `depmod` over the extracted archive. Keep these inputs and extra generated outputs out of the image. Preserve `modules.builtin.bin` and `modules.builtin.alias.bin` byte-for-byte. Permit other index changes only for the approved module delta, with dependency and alias resolution checked.

Before applying diagnostics, prove an unmodified control through index regeneration, extraction, and repacking. Require unchanged indexes and preserve every member's payload, type, permissions, UID/GID, link target, and hardlink relationships. The saved image has seven early members, 1,162 main members, and gzip starting at byte 10,240. Preserve that archive split, member placement, duplicate handling, and early segment; a merged-directory comparison is insufficient. Verify these boundaries and compression with the installed tools before choosing the exact repack command. No archive-member path may escape the new private directory. Keep embedded `buildconfig` unchanged as base-image provenance; retain new build/repack provenance externally.
...
- The new image's exact hash, size, member/delta manifest, and source/patch/build records are pinned. The proposed destination is `/boot/initramfs-linux-asahi-dpalt-usbdiag1.img`; it has not been created or selected. An existing destination is a stop, not permission to overwrite.

D2 requires a new reviewed, fixed-source/fixed-destination staging-only helper. Preserve the original atomic no-replace copy, before/after protected hashes, permissions, sync, and incomplete-result rules. Add protection for the working DP image and both installed USB modules. David alone runs the privileged command. Staging success does not authorize reboot.
