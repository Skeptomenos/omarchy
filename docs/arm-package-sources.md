...
The ARM fork retains a checkout-key fallback that diverges from upstream's package-only migration gate. At the 2026-09-19 integration, the fork's `/edge` repository has no `omarchy-mac-keyring` package; `/rc` provides `20260914-2`. Bootstrap migration `1789316115.sh` first attempts installation through the normal package helper. Both it and successor `1789407945.sh` require `20260914-2` or newer when the package is installed, then populate its trust and verify the new fingerprint. An installed older package fails the migration; it cannot select the fallback.

When the package is absent, both migrations use `$OMARCHY_PATH/default/pacman/keyrings/omarchy-mac.gpg`. They require a regular, non-symlink file with exactly one public primary whose fingerprint is `FBD6874D423C418DDB6D143EECE19CDDE306DBD2`. They reject any secret primary or secret subkey records before import. They then import the public key, verify the imported fingerprint, and locally sign that primary. They never fetch this key from a keyserver or change repository signature policy. Package installation, version comparison, key inspection, population, import, fingerprint verification, and signing failures leave the migration pending and stop later markers. Old and unrelated installed trust remains intact.

The successor applies this repair to hosts that already completed `1789316115.sh`. Upstream's new `1789407944.sh` is imported as `1789407945.sh`, an unused ID above both merge parents' migration maxima. The existing bootstrap ID stays unchanged. Publish the reviewed keyring package with the RC4 pair for packaged clients; the checkout fallback does not qualify package delivery or a release.
