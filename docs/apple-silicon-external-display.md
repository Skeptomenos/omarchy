...
The [one-boot diagnostic plan](plans/dev-147-usb-startup-diagnostic.md) passed design review. D0 is complete. D1 now has 59 passing trace tests, 58 archive-helper tests, compiled private controls/diagnostics, exact import checks, and bounded userspace logging QA. The [real-control record](evidence/dev-147-real-archive-controls-2026-08-28.md) records byte-identical image/index reconstruction and binary-only lookup for all 199 original modules. It preserves the format and scratch-output failures and their narrow corrections. Next is a separate private diagnostic image and its delta checks. No diagnostic image exists yet. Staging, live load, and reboot remain unauthorized. The monitor is not needed for offline D1; a permitted unplug was not confirmed or counted as a test.
