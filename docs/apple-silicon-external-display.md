...
The [one-boot diagnostic design](plans/dev-147-usb-startup-diagnostic.md) passed independent review. D0 is complete, and David approved offline D1 implementation, focused QA, and private module/image preparation. R4 passed its initial isolation check. Fresh isolated fixture runs now establish the expected RED state of the unimplemented trace and image helpers; they do not validate an implementation. The [source checkpoint](evidence/dev-147-public-source-checkpoint-2026-08-28.md) records that boundary. Next are implementation, focused tests, independent review, and private control/diagnostic builds. No diagnostic module or image has been built. No staging, live load, or reboot is authorized. The monitor is not needed for this offline work and may be unplugged; an unplug was not confirmed or counted as a test.
