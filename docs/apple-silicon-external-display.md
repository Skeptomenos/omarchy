...
> [!WARNING]
> This DEV-147 prototype remains experimental work in progress. Native external video was verified in one live session, one candidate startup, and one reconnect on 2026-08-27. The reconnect also restored monitor USB devices. Overall startup acceptance remains on hold for automatic USB enumeration and recorded diagnostics. Reliability, full rollback, and permanent installation are not proved. Do not use it on another machine or kernel.

Reconciled: 2026-08-28, after offline helper QA. The [living execution plan](plans/dev-147-m2-displayport.md) supersedes this guide's former gate/status instructions. Read it before any next action. The [dated history](evidence/dev-147-prototype-history-2026-08-27.md) preserves the attempts, holds, corrections, and success. The old “output is not confirmed” checkpoint remains on the retained private branch at `8db5ef5b` and in the original private issue history; it described the pre-Gate-3 state.
...
The [one-boot diagnostic plan](plans/dev-147-usb-startup-diagnostic.md) passed design review. Offline D1 is complete: 59 trace, 58 archive, and 55 assembly tests pass. Private controls/diagnostics compile; exact import and bounded userspace logging checks pass. The [real-control record](evidence/dev-147-real-archive-controls-2026-08-28.md) preserves byte-identical base reconstruction and all 199 original lookups. The [image record](evidence/dev-147-private-diagnostic-image-2026-08-28.md) records the retained assembly failures, corrections, and one verified private diagnostic image: 200 modules, exactly four archive changes, and 413 successful child commands. Next is approval for a separate staging-only helper. Nothing was staged or loaded, and no reboot occurred. The monitor was not needed for offline D1; a permitted unplug was not confirmed or counted as a test.
