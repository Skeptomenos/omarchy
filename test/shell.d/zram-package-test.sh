...
! grep -Fx 'systemctl start systemd-zram-setup@zram0.service' "$calls" >/dev/null ||
  fail "the zram migration does not restart an active zram device"
! grep -q '^sudo ' "$calls" ||
  fail "the zram migration does not request privileges when zram is active"
pass "the zram migration leaves an active machine unchanged"
