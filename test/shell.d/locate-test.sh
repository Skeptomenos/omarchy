...
check(not (root / "install/config/locate.sh").exists(),
      "the retired locate configuration helper is absent")
legacy_migration = root / "migrations/1784809451.sh"
for directory in ("bin", "install", "migrations"):
  for path in (root / directory).rglob("*"):
    if path.is_file() and path != legacy_migration:
...
check(True, "only the retained historical migration references the retired configuration helper")
...
with tempfile.TemporaryDirectory(prefix="omarchy-locate-") as scratch:
  ...
  metadata = conf.stat()
  subprocess.run(["bash", "-euo", "pipefail", str(legacy_migration)],
                 env=dict(env, OMARCHY_PATH=str(root), OMARCHY_UPDATEDB_CONF_PATH=str(conf)), check=True)
  check(conf.read_bytes() == original and not calls.exists(),
        "the retained migration ID is inert after retirement of its helper")
