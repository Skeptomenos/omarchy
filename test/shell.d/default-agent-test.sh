...
: >"$stub_log"
OMARCHY_TEST_MISSING_COMMAND=muse source "$ROOT/migrations/1789444034.sh" >/dev/null
grep -Fx "$muse_package muse" "$stub_log" >/dev/null || fail "Muse migration creates its lazy stub"
: >"$stub_log"
source "$ROOT/migrations/1789444034.sh" >/dev/null
...
source "$ROOT/migrations/1789444029.sh" >/dev/null
...
source "$ROOT/migrations/1785846769.sh" >/dev/null
source "$ROOT/migrations/1787342993.sh" >/dev/null
OMARCHY_TEST_MISSING_COMMAND=cursor-agent source "$ROOT/migrations/1789444029.sh" >/dev/null
...
#!/bin/bash
[[ $1 == openclaw && ( ${OMARCHY_TEST_OPENCLAW_INSTALLED:-false} == "true" || -f $OMARCHY_TEST_STUB_LOG.openclaw ) ]]
SH
cat >"$mock_bin/omarchy-pkg-add" <<'SH'
#!/bin/bash
printf '%s\n' "pkg-add $*" >>"$OMARCHY_TEST_STUB_LOG"
touch "$OMARCHY_TEST_STUB_LOG.openclaw"
SH
