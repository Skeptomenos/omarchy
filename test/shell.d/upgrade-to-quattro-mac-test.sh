...
grep -F 'upgrade_ref="${OMARCHY_UPGRADE_REF:-quattro-arm}"' "$upgrade_to_quattro_mac" >/dev/null ||
  fail "Mac Quattro upgrade defaults to the quattro-arm ref" "expected: upgrade_ref=\"\${OMARCHY_UPGRADE_REF:-quattro-arm}\""
pass "Mac Quattro upgrade defaults to the quattro-arm ref"
