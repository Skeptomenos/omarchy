...
migration="$ROOT/migrations/1789444045.sh"
...
printf '%s\n' linux-asahi linux-omarchy linux-t2 > "$INSTALLED_PACKAGES"
: > "$CALL_LOG"
TEST_ARCH=aarch64 bash -euo pipefail "$migration" >/dev/null
[[ ! -s $CALL_LOG ]] || fail "ARM must never receive x86 kernel headers"
pass "ARM skips x86 headers even when stale x86 package records exist"
