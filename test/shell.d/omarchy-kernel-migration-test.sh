...
migration="$ROOT/migrations/1789444044.sh"
scratch=$(mktemp -d)
...
! grep -Eq 'omarchy-pkg-add.*linux-omarchy-ptl' "$ROOT"/migrations/*.sh ||
...
[[ $pending == "1789444044.sh" ]] || fail "the renamed migration is pending after completing the old migration"
...
[[ -f $OMARCHY_KERNEL_REBUILD_MARKER && -f $OMARCHY_MIGRATION_STATE/1789444044.sh ]] || fail "new machine and user completion markers are recorded"
assert_preferred "$OMARCHY_KERNEL_LIMINE_CONF"
