...
clone_repo_to_user() {
    local username="$1"
    local repo="${OMARCHY_REPO:-Skeptomenos/omarchy-mac}"
    local ref="${OMARCHY_REF:-quattro-arm}"

    print_step "Cloning Omarchy Mac into user's home"
    su - "$username" -c "bash -lc 'set -e; mkdir -p ~/.local/share; rm -rf ~/.local/share/omarchy; git clone --branch \"${ref}\" --single-branch https://github.com/${repo}.git ~/.local/share/omarchy'"
