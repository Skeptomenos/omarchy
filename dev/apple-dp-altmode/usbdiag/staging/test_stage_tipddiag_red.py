"""A3 T1 staging RED: preserve 38 historical real-file methods and add T1 gates.

Run only in the reviewed unprivileged sandbox with four fixed read-only files.
The incomplete subject is exact C3 plus one documented no-op power interface.
Select only the three reviewed RED methods. Successful authentication and
collectors must precede exactly three assertion failures, not setup errors.
No production preflight or real image is used. Private synthetic files cannot
prove root staging, hardware safety, physical MagSafe or active charging.
...
HELPER_SHA256 = "d7cdd558519cc984ea7209aacfb0d9fb3be6a5eafb544374fd1d20885554dadd"
