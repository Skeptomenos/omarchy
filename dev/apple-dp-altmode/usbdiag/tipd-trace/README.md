...
- `validate_capture(document)` has no caller-supplied manifest, expected hash, or environment override. This legacy entry always returns `artifact_binding_unavailable` and retains no fixed artifact inputs. The separate capture layer below records the accepted TIPD SHA and enforces its before/after build-ID note. Full module-SHA/image binding and operational activation remain unavailable; a fixture binding can never activate either entry.
...
## Bounded capture-package checkpoint

`capture_binding.py` checks bounded caller-submitted journal stdout, stderr, a strict receipt, and exact before/after boot-ID and module-note samples. It fixes kernel `7.1.6-1-1-ARCH`, records candidate module SHA-256 `a0fdadc351261643545e5afd8561923be99431661c447084336c9318f5b0c02f`, and enforces build ID `40aa54382047ba36b02c9ac0da65a213862a77ad` through the note samples. It rejects duplicate JSON keys, non-kernel transport, filtered priorities, receipt/hash drift, boot drift, module-note drift, and malformed or mixed T1 record families. An incomplete trace returns `inconclusive`; a valid cap returns `limited`. A successful complete result remains `internally_consistent_only` with every operational/hardware claim false. Full module-SHA/image provenance remains a later gate.

`collector_recipe.py` exposes only the exact all-priority `journalctl` command plan and fixed bounds. `collect_capture()` always stops with `artifact_binding_unavailable`. Neither module reads a journal, path, environment variable, device, or sysfs attribute.

Independent review found two regressions before final acceptance: duplicate keys could hide an escaped T1 revision during family classification, and boot/module-note samples lacked size checks before hashing. The retained review RED has exactly those two assertion failures. The corrected final publication package passes all 21 methods with 586 read-only bindings, unchanged inputs, no timeout, and zero workload children. Independent static and actual-output reviews pass. Accepted pins are contract `34137a59ac44f1790d6ab18809a4a71eaa2b6e1e40724a38bc1e5c4a3fdca87e`, binding `09a2a4d2631c64ac428d3071b4c9dabbe79b7fb4ac8b2f5682fba9506b8263ce`, collector `911aeeb2c464b3514280f74a78961f190fe4d7d86122e2a014343b4f1604a40e`, runner `6d052b0ab1735fded9da2619a37a683a9a60dabbecc4353e6b269378307c33a5`, parser `8c1e90a30f68c9237948e47f583038aee0d4584fa2459779e518b1630372e0fe`, and tests `2237cb697530556f811eae873e044edcc72f35bfc58e3ce247d20c911343c400`.

This checkpoint is not a live collector or operational image binding. It proves no selected initramfs, earliest-startup attribution, receiver delivery, USB result, charging result, display result, or hardware safety. Activate the fixed artifact/image provenance and collector only after the T1 image is accepted and before the manual boundary.
