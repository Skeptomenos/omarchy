...
The [T1 private-candidate evidence](../../../../docs/evidence/dev-147-t1-private-candidate-2026-08-29.md) records retained source and two byte-identical contained module builds. The accepted private module SHA-256 is `a0fdadc351261643545e5afd8561923be99431661c447084336c9318f5b0c02f`. It was never an E-control input. No T1 image, load, stage, boot, or hardware result exists. Assembly remains on HOLD until a fresh E-control PASS.

The fixture module payloads are labelled ASCII data, not ELF modules. A successful raw delta check proves structure and an explicitly expected payload only. It does not authenticate T1. The fresh E-control proof hash remains unknown and unset. Two fixed production recipe attempts ran all 424 children and failed closed before publication. The first exposed a gzip timestamp mismatch. The second proved the `/usr/bin/gzip -n` correction and then exposed the exact trailing-space grammar of kmod 34.2 `insmod` output. There is no argument, environment variable, CLI option, fake identity, or fixture mode that enables assembly. The later reviewed assembler must authenticate actual bytes and proofs independently before it uses the pure contract.
...
The current [E-control structural boundary](e-control-operational-red-contract.md) separates the accepted zero-child RED, the rejected first GREEN and the accepted corrected structural GREEN. The current structural regression `run-f0tjlamv` passes 3/3 after two retained repository-mode setup stops. Structural output is distinct and always states zero control-workload children and no fresh-control proof. That structural checkpoint had no production entry. A later production candidate ran once. It did not publish a fresh proof.
...
The [fixed E execution boundary](e-control-execution-red-contract.md) has two fail-closed production attempts. `run-f2yoto48` exposed gzip MTIME drift. After that correction passed independent review, fresh probe `run-x65x28u0` passed and the one authorized corrected attempt, `run-noq24xg7`, completed all 424 children. Its gzip output matched retained E exactly. Validation then stopped at the first ordered lookup because kmod 34.2 emits exactly one trailing ASCII space on each `insmod` line. No header, evidence, pending result, final result, or fresh proof was written. The helper now requires that exact space, rejects missing or doubled spaces, and keeps `builtin ecb` unspaced. Current recipe SHA is `1be81904b29c69cb24fae86208c62fb30e830eca46250978a65d8ed19db8de77`; helper SHA is `686d59e63166df1bef1afad27998a6d58f4c28b6b4439b6ccd607b56471268ca`. Controlled helper RED `run-mnmz924l`, helper GREEN `run-2f6yexwm`, and four zero-production-child GREEN runs are recorded in the [second-attempt and lookup-correction evidence](../../../../docs/evidence/dev-147-e-operational-second-attempt-lookup-grammar-2026-08-30.md). [Independent QA](../../../../docs/evidence/dev-147-e-lookup-correction-independent-qa-2026-08-30.md) repeated the helper suite at 18/18 and the four boundary suites at 25/25. Final review passed with no blockers. The [first-attempt evidence](../../../../docs/evidence/dev-147-e-operational-first-attempt-gzip-determinism-2026-08-29.md), [round-3 pre-execution evidence](../../../../docs/evidence/dev-147-e-operational-preexecution-gate-2026-08-29.md), and earlier [execution-boundary evidence](../../../../docs/evidence/dev-147-e-execution-boundary-2026-08-29.md) remain retained history. There is no fresh E-control PASS. Any further production attempt needs a separate GO and fresh v5 probe. Every live and manual boundary remains on HOLD.
