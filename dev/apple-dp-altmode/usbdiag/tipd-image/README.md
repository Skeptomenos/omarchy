...
The fixture module payloads are labelled ASCII data, not ELF modules. A successful raw delta check proves structure and an explicitly expected payload only. It does not authenticate T1. Actual T1 SHA/build ID and the fresh E-control proof hash are unknown and unset. The real operational gate is not implemented. There is no argument, environment variable, CLI option, fake identity, or fixture mode that enables assembly. The later reviewed assembler must authenticate actual bytes and proofs independently before it uses the pure contract.
...
The [fixed E execution boundary](e-control-execution-red-contract.md) is also accepted at zero-child scope. RED `run-0zk61la1` failed only on the three missing boundaries. First GREEN `run-_l2w9p_k` retained a Python 3.14 `Path`/`PosixPath` contract stop. Corrected GREEN `run-nr5woop4` passes 3/3 with 594 read-only test bindings, zero workload children, unchanged inputs, and no real output. The accepted subject SHA is `39496435f113c7d9256e5592effd3fece8c52b0e61b774e8283fe96eb84d4add`. Its fixed production model is eight inputs and 593 read-only bindings. The [dated evidence](../../../../docs/evidence/dev-147-e-execution-boundary-2026-08-29.md) preserves results and limits. The next gate is an independent pre-execution review before one fixed 424-child offline E control. All operational APIs and every live/manual boundary remain closed.
