...
The fixture module payloads are labelled ASCII data, not ELF modules. A successful raw delta check proves structure and an explicitly expected payload only. It does not authenticate T1. The module identity was accepted separately: SHA-256 `a0fdadc351261643545e5afd8561923be99431661c447084336c9318f5b0c02f`, build ID `40aa54382047ba36b02c9ac0da65a213862a77ad`. The T1 image identity and fresh operational E-control proof remain unavailable. The operational gate always refuses. There is no argument, environment variable, CLI option, fake identity, or fixture mode that enables assembly. A later reviewed assembler must authenticate the accepted module bytes and operational proof independently before it uses the pure contract.
...
| Unbound operation | The accepted module identity alone cannot enable assembly. T1 image and operational proof identities remain unset. |
...
The real reduced-root depmod outputs, all 200 binary-only filename/dependency lookups, complete alias/symbol mappings, concrete frontend/DWC3/ATC aliases, TIPD export resolution and unchanged inputs need separate evidence. Preserve the existing exact generated-symbol versus retained-symbol exception; do not copy generated indexes into T1. Separate scoped binary review already accepts ELF/BTF, source-derived imports/exports, unchanged frontend/shared layouts and the wrapper allocation contract within its stated limits. These fixtures do not prove those facts, and a strict header or metadata equality cannot replace the accepted evidence.
...
## Pure E-control recipe checkpoint

`run_e_control.py` adds pure selection, model, plan, and proof-boundary functions. It fixes retained E, its early/main streams, 200 module payloads, seven original index hashes, three index-source inputs, all dependency/alias/symbol expectations, and the exact 424 child-command plan. It reads no path and launches no command. Operational control and T1 assembly remain closed.

After a preserved three-assertion recipe RED, the final isolated recipe package passes all 16 methods with 605 read-only bindings, unchanged inputs, no timeout, and zero workload children. Independent pre-execution and output review pass. The accepted subject SHA-256 is `4058fd04de35307a7321c4254efccb55e8089aa571fdddb63ad383a67bbbd207`; the accepted runner SHA-256 is `5e76f3a01ada88c790b580be197808af6df708e1d280b78f58ef60a7d7c9f5fa`.

This result authenticates the pure model and plan only. The historical generated files are test fixtures. No fresh depmod, lookup, compression, E reconstruction, image creation, module load, staging, or boot occurred. The next gate must execute the fixed 424-command control with independently reviewed result-last acceptance before T1 assembly.
