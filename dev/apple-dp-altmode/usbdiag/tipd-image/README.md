...
The [fixed E execution boundary](e-control-execution-red-contract.md) failed closed on its one production attempt, `run-f2yoto48`. All 424 children completed, but child 4 used bare `/usr/bin/gzip`. Its output differed from retained E only in the four-byte gzip modification-time field. No header, evidence, pending result, final result, or fresh proof was written. The corrected exact command is `/usr/bin/gzip -n`. The SWE zero-child runs and independent QA each pass the same 25/25 focused methods against corrected subject SHA `57d35a30de9b351bcbaf0b78a1be186c8c44a2fbfb378d8f0b801e6e9256a7a9`. Independent direct-helper QA also passes 18/18 methods with 11 bounded fixture children. The fixed production model remains eight inputs and 593 read-only bindings. The [failed-attempt and correction evidence](../../../../docs/evidence/dev-147-e-operational-first-attempt-gzip-determinism-2026-08-29.md) owns the current state. The [round-3 pre-execution evidence](../../../../docs/evidence/dev-147-e-operational-preexecution-gate-2026-08-29.md) and earlier [execution-boundary evidence](../../../../docs/evidence/dev-147-e-execution-boundary-2026-08-29.md) remain retained history. Independent QA is complete. The next conditional gate is independent final review. A second production attempt then needs a separate GO and fresh v5 probe. Every live and manual boundary remains on HOLD.
