...
At the first checkpoint, the then-current [run_e_control.py](run_e_control.py) was the tested pure post-RED implementation. `select_e()` bound the one complete E byte string before it parsed the exact streams, 200 modules, four named payloads and seven indexes. The sorted 200-entry name/path model had fixed SHA `eee8ad06a36c1537d53e0c416db998110d10638076a32bdd3fc8987f65b54bff`; a caller could not exchange one unrelated but well-shaped module path. `validate_regeneration()` required the fixed eleven outputs and full binary dump, checked the complete dependency model and mapping multiplicities, and preserved the one reviewed `modules.symbols.bin` distinction. `command_plan()` returned only the fixed 424 command tuples. These pure functions had no file, environment or subprocess API. At that checkpoint, `main()` remained unconditionally closed.

The historical full companion has 16 methods. It covers the exact E selector, base hash/size/type rejection, the seven original and eleven generated index identities, strict index membership/types, the one reviewed symbol-index distinction, exact dump bytes, complete dependency-model membership, exact command sequence, invalid names/placement, and the then-closed operational/assembly gates. Regression `run-m64c0_of` passed 16/16 methods with 605 unchanged read-only bindings, no timeout, and zero children. Its result explicitly records `fresh_control_proved=false`, `image_created=false`, `module_loaded=false`, `staged=false`, and `booted=false`. The earlier GREEN `run-7vguug70` and RED `run-9fmdox3j` remain preserved. These runs wrote only fixture metadata under the fresh `/work/e-recipe-fixtures` directory. This pure fixture GREEN does not claim fresh depmod, lookup, gzip, image, or hardware success.

## Narrow read-only fixture bindings

The first checkpoint used the unchanged reviewed A2 runtime. These were 20 inputs: seven source/runner bindings, the fixed E image, and twelve individual historical files. The expected read-only mount count was 605: 582 runtime entries, three fixed sandbox files and 20 inputs. `/work` and `/tmp` were the only writable roots. The 280-second workload and 285-second outer limits remained unchanged. No manifest repin, installation, old launcher, or unrestricted fallback was permitted.

| Sandbox binding | Input |
|---|---|
| `/inputs/test` | New `test_e_control_recipe.py`, pinned by the outer launcher. |
| `/inputs/recipe` | Historical tested subject SHA `70f369f87942b6ca6826c808536353ae0cc400123204040b9c005995ab43c3e3`. |
| `/inputs/subject/e_control.py` | Historical tested helper SHA `abbf59410a05fd5c789820df3d40e59d0a5c33cf1204ab93c7aeef806da7b1df`. |
...
| GNU gzip | One exact `/usr/bin/gzip -n` child with the checked regular main-stream file as stdin. Bare gzip is refused. Require the exact reconstructed E bytes, not only a successful decompression. Do not retry another flag, compressor, or input layout. The retained child stdout is sufficient; no `.img` output is needed. |
...
| 4 | Exact `/usr/bin/gzip -n`, checked regular-file stdin. | 1 |
