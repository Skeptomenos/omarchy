...
## Current corrected E control: completed evidence gate

This fixture GREEN covers these boundaries only. The actual E control remained a separate evidence gate. Its first two production attempts, `run-f2yoto48` and `run-noq24xg7`, completed all 424 children and failed closed before publication. The first exposed gzip MTIME drift. The second proved the gzip correction and exposed lookup-line grammar. Fresh probe `run-y3xcwg2_` and the one authorized third attempt `run-988kuwr1` then passed. The third attempt authenticated exact E SHA `4d4f0557af57eebcc33322f004bcc7968254e644b069a11102375df0b31a52ae`, 19,191,513 bytes. It retained the original TIPD payload and proved the exact early/main streams, all 200 module identities, raw no-op reconstruction, and byte-exact GNU gzip output. The [image contract](README.md#gates-not-supplied-by-these-fixtures) retains the fixed stream hashes and limitations. The [first-attempt evidence](../../../../docs/evidence/dev-147-e-operational-first-attempt-gzip-determinism-2026-08-29.md), [second-attempt evidence](../../../../docs/evidence/dev-147-e-operational-second-attempt-lookup-grammar-2026-08-30.md), and [third-attempt evidence](../../../../docs/evidence/dev-147-e-operational-third-attempt-pass-2026-08-30.md) own the history and final result.
...
Before the completed execution, a separate review confirmed every path, tool/source/input pin, complete alias/symbol normalization and multiplicity check, result schema, and immutable-root readback. The run preserved the exact reviewed generated-symbol-index exception. It did not replace any of the seven final E index bytes. The refusal remains active: unexpected command count, output, ordering, index, or source drift means HOLD. The completed gate does not permit adaptive retries, repinning, text fallback, a broader delta, or automatic T1 assembly.

The final complete control proof binds the actual raw archive, module, index, lookup, child, and before/after root records. The result was written only after all checks passed; the strict header alone did not satisfy the gate. Accepted T1 binary identity and the complete fresh E-control proof still remain unbound assembly inputs. No candidate image, staging helper, privileged preflight, module load, cable action, or boot is authorized by this fixture contract.
