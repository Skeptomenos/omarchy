...
Reconciled: 2026-08-28, after one successful working-image display recovery. The [recovery record](../../../docs/evidence/dev-147-dp-recovery-2026-08-28.md) owns that result; the [D3 failure record](../../../docs/evidence/dev-147-usbdiag-startup-failure-2026-08-28.md) retains the failed diagnostic startup and unfixed logging-defect proof. Recovery does not validate this diagnostic archive or close USB acceptance.
...
No public install or quick-start command is provided. Tests and builds need a separately reviewed private continuation with pinned read-only inputs and fresh writable output/temp directories. Do not weaken a guard, create an unrestricted fallback, or rerun a frozen checkpoint's launcher. D2 staging is complete. The [D3 readiness handoff](../../../docs/evidence/dev-147-usbdiag-boot-readiness-2026-08-28.md) was consumed by the failed diagnostic startup. The main plan's [working-DP recovery handoff](../../../docs/plans/dev-147-m2-displayport.md#current-recovery-handoff--previous-working-dp-image-living) is also consumed: both screens work with packaged USB drivers and the working patched DP core, but only root hubs enumerate. Do not repeat either handoff. The [next step](../../../docs/plans/dev-147-m2-displayport.md#next-step--offline-correction-design-only-living) is an offline correction-design proposal only. No implementation, test run, rebuild, new reboot, hotplug, USB-device test, live swap, mode change, or suspend is authorized.
