...
|---|---|---|
| [kernel](kernel/) | Instrumented `dwc3-apple.c`, `atc.c`, and the matching draft patch | UNBUILT. No compiler, ABI, cap-concurrency, or hardware validation is claimed. |
| [trace](trace/) | Event schema, typed validator API, synthetic capture fixtures | UNIMPLEMENTED stub. Initial isolated RED: 32 tests, 40 NotImplementedError errors. No GREEN result. |
...
The errors above are expected evidence of missing implementation, not passing tests. Both initial RED runs used frozen private stubs in a fresh, reviewed sandbox. Inputs remained unchanged and neither run timed out. They did not execute these edited public helpers.
...
The proposed records use revision `dev147-usbdiag1-v1`, board `j413`, and target `front_lower`. They filter `/soc/usb@502280000` and `/soc/phy@503000000`. Fixed INFO JSON should record operation order without changing the original hardware calls, error handling, lock order, retries, or timing policy. The per-component limit is 128 records including a final cap marker, with a 384-byte record bound. Those implementation properties still require independent validation.
