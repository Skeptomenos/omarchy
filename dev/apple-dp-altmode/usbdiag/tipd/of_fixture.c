...
static void make_tree(void *blob, int bytes, unsigned int kind)
{
  ...
  const char *board = kind == 1 ? upper : kind == 2 ? wrong : good;
  int length = kind == 1 ? sizeof(upper) : kind == 2 ? sizeof(wrong) : sizeof(good);
  ...
  fdt_check(fdt_begin_node(blob, "soc"));
  ...
  if (kind != 4) {
    const char *name = kind == 5 ? "usb-pd@03f" :
      kind == 6 ? "usb-pd@3F" : kind == 7 ? "usb-pd@3f-extra" : "usb-pd@3f";
...
static struct device_node *metadata_fixture(unsigned int kind)
{
  ...
  if (kind == 4 || kind == 10) selected = NULL;
  if (kind == 11) of_root = NULL;
  metadata_released();
  return selected;
}
