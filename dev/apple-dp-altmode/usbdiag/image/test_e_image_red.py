"""E-only archive contracts against the unchanged, authenticated old assembler.

These tests do not call assembly main, run depmod, compress an image, or load a
module. Real newc records exercise the existing parser and archive-delta gate.
Module payloads used for DWC provenance are pinned real binaries. Small index
payloads exercise archive identity only; they do not prove binary-index content.
The retained real dependency/alias validators also get bounded text fixtures.

The approved no-install exception uses typed stdlib/unittest, not new packages.
The first selected positive must fail at the old ATC-replacement requirement,
not at import, source authentication, fixture parsing, or a missing input.
"""
...
SOURCE_SHA256 = "00caceb3b7fa236dcc030fb4007d0baa75bfa08fcd1590626f85fcc8c22d5f60"
...
class IndexSemanticTests(unittest.TestCase):
  def test_real_dependency_and_alias_validators_accept_only_the_e_delta(self) -> None:
    ...
    subject.validate_dependency_delta(before, after, original)
    aliases = subject.ALIASES_HEADER + b"alias of:N*T*Cfixture,existing phy_apple_atc\n"
    added = ("\n".join(sorted(subject.DWC_ALIASES)) + "\n").encode("ascii")
    subject.validate_alias_delta(aliases, aliases + added)
    self.assertEqual(Counter(subject.alias_entries(aliases + added, subject.ALIASES_HEADER)),
                     Counter(subject.alias_entries(aliases, subject.ALIASES_HEADER)) + subject.DWC_ALIASES)
    for bad in (after.replace(f"{ATC}:\n".encode(), f"{ATC}: {TIPD}\n".encode()),
      ...
      with self.subTest(dependency=bad[-100:]), self.assertRaises(RuntimeError):
        subject.validate_dependency_delta(before, bad, original)
    for bad in (aliases, aliases + added + added, aliases + added + b"alias unexpected other\n"):
      with self.subTest(alias=bad[-100:]), self.assertRaises(RuntimeError):
        subject.validate_alias_delta(aliases, bad)
