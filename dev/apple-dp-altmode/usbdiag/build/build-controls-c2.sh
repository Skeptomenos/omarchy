...
# Fresh private C2 workload only. The launcher owns containment and its deadline.
...
printf '%s  %s\n' \
  ...
  d150400f9782c876972b2745d95617cd44e23574452f63980704911a467f7767 /inputs/stock/dwc3-apple.ko \
  fd1c3d105bd69a649a38e89e2ca0bcbe6f656200a0f211d58211e8c7b3ec944b /inputs/stock/phy-apple-atc.ko \
  | /usr/bin/sha256sum --check --strict

# Authenticated inputs stay read-only; all outputs belong to this fresh run.
...
for module in dwc3-apple phy-apple-atc; do
  ...
  for field in name vermagic depends alias; do
    [[ $(/usr/bin/modinfo -F "$field" "$module.ko") == $(/usr/bin/modinfo -F "$field" "/inputs/stock/$module.ko") ]] || {
      printf 'STOP: control %s differs in %s\n' "$module" "$field" >&2
      exit 1
    }
  done
  /usr/bin/nm -u "$module.ko" | /usr/bin/sort >"$module.imports"
  /usr/bin/nm -u "/inputs/stock/$module.ko" | /usr/bin/sort >"$module.stock-imports"
  /usr/bin/cmp "$module.imports" "$module.stock-imports"
done
printf 'CONTROL BUILD AND BASIC ABI CHECKS PASS; independent C2 review pending; no module loaded\n'
