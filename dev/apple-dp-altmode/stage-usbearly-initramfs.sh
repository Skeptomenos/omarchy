...
# C3 E-only staging source. Public copies have invalid host and source constants.
...
readonly D2ST_SOURCE="/LOCAL_ONLY_DEV147_C2/sandbox-tools/run-affn1zit/work/initramfs-linux-asahi-dpalt-usbearly1.img"
readonly D2ST_PROOFS="/LOCAL_ONLY_DEV147_C2"
readonly D2ST_DESTINATION="/boot/initramfs-linux-asahi-dpalt-usbearly1.img"
readonly D2ST_IMAGE_SHA="4d4f0557af57eebcc33322f004bcc7968254e644b069a11102375df0b31a52ae"
readonly D2ST_IMAGE_SIZE=19191513
...
d2stage_require_operational() {
  [[ $D2ST_ROOT_UUID =~ ^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$ &&
    $D2ST_PROOFS == "/home/david/o/.dev147-stage/usbdiag-c2-20260828.5p4qLBgKvH" &&
    $D2ST_SOURCE == "$D2ST_PROOFS/sandbox-tools/run-affn1zit/work/initramfs-linux-asahi-dpalt-usbearly1.img" ]] ||
...
d2stage_finish() {
  ...
  printf 'STAGING ONLY PASS: %s\nChecks retained in %s\nNo reboot permission. Normal boot is unchanged; this uninstrumented early-availability image is untested at startup.\n' "$destination" "$directory" || d2stage_die "cannot report completion"
}
...
d2stage_proof_inputs() {
  printf '%s  %s\n' \
    "b3aa81bfcd2946869d537526ab557fe1b0d9ab630167884ab639d7bab5345f5c" "$D2ST_PROOFS/HANDOFF.md" \
    "0a810bea90ff983019cb6c717baeddc490955961a8a310cf1d13a2d2fd3bba7e" "$D2ST_PROOFS/SHA256SUMS" \
    "e7eeb46796f1f04a423b661f7ec3fa9e9655c2a291d034d8c7ff48753849b302" "$D2ST_PROOFS/seal-verification.json" \
    "f2a186513a36ce68441cfe79511b6635f906e497dfd5ab8064bcaee840d4c46d" "$D2ST_PROOFS/e-independent-qa.json" \
    "1e44b1f4b212c931a86972bff828cb8cd64c005e95634e444e980d18f181f79d" "$D2ST_PROOFS/c2-final-review.json" \
    "80df8c3f723c84e8c885550e0b32c808cd5488ba3e4cb5921a4f20039e3e9d29" "$D2ST_PROOFS/sandbox-tools/run-affn1zit/work/e-assembly-result.json" \
    "058a0f0288540eb6b2af0c5876bad783457af668db0bd8d8f28a0789da7c93f2" "$D2ST_PROOFS/sandbox-tools/run-affn1zit/work/e-image-delta.json" \
    "995626ca50174bc34f03fdf59825ddb8c485ffcb59d62d20116b394620ac3a1f" "$D2ST_PROOFS/sandbox-tools/run-affn1zit/result.json" || d2stage_die "cannot produce C2 proof records"
}
...
d2stage_main() {
  ...
  private_directory=$(d2stage_clean /usr/bin/mktemp -d /boot/.dev147-usbearly-stage.XXXXXXXXXX) || d2stage_die "cannot create private staging directory"
  ...
  d2stage_check_space /boot "$D2ST_IMAGE_SIZE" "$D2ST_RESERVE"
  temporary="$private_directory/.initramfs-linux-asahi-dpalt-usbearly1.img.tmp"
