...
class StageTests(unittest.TestCase):
  def setUp(self) -> None:
    ...
    for name in ("kernel-source-config.json", "boot-bundle.json", "initramfs.json"):
  ...
  def test_stage_preserves_active_system(self) -> None:
    ...
    self.assertEqual(
      (self.root / f"boot/dev147-fairydust-{RELEASE}/Image").read_bytes(),
  ...
  def test_symlink_target(self) -> None:
    (self.root / f"boot/dev147-fairydust-{RELEASE}").symlink_to("grub")
    self.run_stage(1)
  ...
  def test_protected_drift_during_copy(self) -> None:
    ...
    copied = self.root / f"var/lib/dev147-fairydust-stage/{RELEASE}/input/Image"
    ...
    self.assertFalse((self.root / f"modules/{RELEASE}").exists())
    self.assertFalse((self.root / f"boot/dev147-fairydust-{RELEASE}").exists())
  ...
  def test_oversized_input_refused_before_content_copy(self) -> None:
    ...
    self.assertFalse(
      (self.root / f"var/lib/dev147-fairydust-stage/{RELEASE}/input/Image").exists()
    )
  ...
  def test_oversized_unlisted_input_refused_before_content_copy(self) -> None:
    ...
    self.assertFalse(
      (
        self.root / f"var/lib/dev147-fairydust-stage/{RELEASE}/input/000-unlisted"
      ).exists()
    )
  ...
  def test_recursive_file_budget_is_shared(self) -> None:
    ...
    result = self.run_stage(1)
    self.assertIn("delivery byte budget exceeded", result.stderr)
    copied = self.root / f"var/lib/dev147-fairydust-stage/{RELEASE}/input"
