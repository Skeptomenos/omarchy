...
Do not execute `stage-image.py` or `stage-image-bootstrap.txt` by pathname. The reviewed handoff requires the literal contents of `stage-image-bootstrap.txt` to be pasted into the terminal. [Staging readiness](../../../docs/evidence/dev-147-afk-reuse-staging-readiness-2026-09-02.md) owns its hashes, tests, review, and unstaged state.
